...
void output_parameters_to_file(std::string &filename, const int argc, const char **argv,  struct Args &args, int cuda_cores)
{
  // Output file
  ...
  output_parameters << "Número de núcleos CUDA: " << cuda_cores << std::endl;
...
// #endif // #ifdef SCICELLXX_USES_MPI
  ...
  output_parameters_to_file(parameters_filename, argc, argv, args, cuda_cores);
  ...
  // Crear un arreglo unidimensional para pasar al dispositivo
  ...
  const unsigned n_configurations_per_core = n_all_configurations / cuda_cores + 1;
  ...
  for (unsigned i_config = 0; i_config < n_configurations_this_core; i_config++)
   {
    ...
    // scicellxx_output<< "Config_id/total: " << configuration_index << "/" << n_all_configurations << "\talpha:" << ss_alpha.str() << "\tbeta:" << ss_beta.str() << "\trho:" << ss_rho.str() << "\tomega_in:" << ss_omega_in.str() << "\tomega_out:" << ss_omega_out.str() << std::endl;
