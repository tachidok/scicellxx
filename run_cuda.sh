...
# Script para compilar y ejecutar la versión CUDA de mTASEP (Rodolfo)

# 1. Rutas
ROOT_DIR="/home/rodolfo/Descargas/mTASEP_CUDA"
BINARY_PATH="$ROOT_DIR/private/rodolfo/mTASEP/bin/rodolfo_demo_mTASEP_LK"
...
if [ "$do_build" = true ]; then
    ...
    cd "$ROOT_DIR"
    ./autogen.sh -c ./configs/advanced/CUDA -d 0
    cd build
    make rodolfo_demo_mTASEP_LK
else
    echo "Saltando compilación (el binario ya existe). Usa ./run_cuda.sh --build para recompilar."
fi

# 4. Preparar carpeta de resultados en el directorio de la demo
cd /home/rodolfo/Descargas/mTASEP_CUDA/private/rodolfo/mTASEP/
output_folder="RESLT_CUDA_0"
rm -rf $output_folder

# 5. Ejecutar
echo "Ejecutando simulación CUDA..."
time /home/rodolfo/Descargas/mTASEP_CUDA/private/rodolfo/mTASEP/bin/rodolfo_demo_mTASEP_LK \
...
--output_space_state_diagram 0 --output_microtubule_state 0 \
--root_output_folder $output_folder --cudablocks 64 --cudathreads 256
